...
//Added in lab 10
const int16_t IR0Weight = -10;
const int16_t IR1Weight = -6;
const int16_t IR2Weight = -3;
const int16_t IR3Weight = 0;
const int16_t IR4Weight = 0;
const int16_t IR5Weight = 3;
const int16_t IR6Weight = 6;
const int16_t IR7Weight = 10;

uint16_t timeNotGreen = 0;

volatile uint8_t sensorValue0 = 0;
volatile uint8_t sensorValue1 = 0;
volatile uint8_t sensorValue2 = 0;
volatile uint8_t sensorValue3 = 0;
volatile uint8_t sensorValue4 = 0;
volatile uint8_t sensorValue5 = 0;
volatile uint8_t sensorValue6 = 0;
volatile uint8_t sensorValue7 = 0;

volatile int16_t errorNumerator = 0;
volatile int16_t errorDenominator = 0;
...
const uint8_t baseSpeed = 55;

volatile int16_t control = 0;

const uint32_t piviotTime = 1500000;

void updateDutyFromError (void);
void clearSensorValueArray (void);
...
void calculateP (void) {
    ...
    errorNumerator = (sensorValue0 * IR0Weight) + (sensorValue1 * IR1Weight) + (sensorValue2 * IR2Weight)
            + (sensorValue3 * IR3Weight) + (sensorValue4 * IR4Weight) + (sensorValue5 * IR5Weight)
            + (sensorValue6 * IR6Weight) + (sensorValue7 * IR7Weight);
    errorDenominator = (sensorValue0 + sensorValue1 + sensorValue2 + sensorValue3 + sensorValue4
    ...
    if (errorDenominator == 0) {
        ...
        lastError = error;
        error = errorNumerator / errorDenominator;
    }
...
void updateDutyFromError (void) {
    ...
    // Convert control values to current speed R and current speed L
    if (leftCount == 3) {
        //do left piviot turn
        Timer_A_stopTimer(TIMER);
        configPWMTimer(PERIOD, CLOCKDIVIDER, 0, LEFTCHANNEL);
        configPWMTimer(PERIOD, CLOCKDIVIDER, slowDuty, RIGHTCHANNEL);
        Timer_A_startCounter(TIMER, TIMER_A_UP_MODE);
        //sofware delay, time it takes to piviot
        __delay_cycles(piviotTime);
    }
    else {
        currentSpeedR = baseSpeed - control;
    }


    //currentSpeedR = baseSpeed - control;
    if (rightCount == 3) {
        //Do right piviot turn
        //do left piviot turn
        Timer_A_stopTimer(TIMER);
        configPWMTimer(PERIOD, CLOCKDIVIDER, slowDuty, LEFTCHANNEL);
        configPWMTimer(PERIOD, CLOCKDIVIDER, 0, RIGHTCHANNEL);
        Timer_A_startCounter(TIMER, TIMER_A_UP_MODE);
        //sofware delay, time it takes to piviot
        __delay_cycles(piviotTime);
    }
    else {
        currentSpeedL = baseSpeed + control;
    }
...
void handleStandBy(void) {
    ...
    LEDStatesControl();
    serviceLEDs();
    __delay_cycles(30000); //Delay time to read
}
